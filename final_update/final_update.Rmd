...
为了衡量任意校区的种族多样性指数，采用的**种族多样性指数**(Diversity Index, 简称 DI)， 定义为校区内多数族裔占学术总数的比例。 DI 越小，表明该校区在种族上越多样：  
...
```{r state-map, cache = T, fig.height = 8.5, fig.width = 8.5, cache = T}
...
tm_shape(map_by_state_2017) + 
  ...
  tm_scale_bar(width = 0.3, text.size = 1.2,
               position = c(0.5, 0.02)) +
  tm_layout(frame = FALSE,
            legend.bg.alpha = 0,
            legend.frame = FALSE,
...
我们分别选出 5 个 “最多样” 和 “最不多样” 的州，一定程度可以印证这种说法，最多样的州（蓝色底色）靠近海岸和边境，不少处于著名的移民城市；而最不多样（红色底色）的州或处于内陆，或人口稀少，经济较为落后：  
...
most_diverse <- diversity_2017 %>% 
  ...
  select(state, st, lea_name, leaid, d_locale_txt, diverse, total, aian:multi)  
...
most_diverse_centroid <- most_diverse_shape %>%
  ...
  select(lea_name, state, diverse, aian:multi) %>%
...
least_diverse <- diversity_2017 %>% 
  ...
  select(state, lea_name, leaid, diverse, d_locale_txt, total, aian:multi)
...
combined_centroid_map <- mapview(state_laea %>% 
          filter(GEOID %in% c(12, 24, 37, 10, 32, 
                              54, 33, 50, 23, 30)) %>% 
    mutate(type = if_else(GEOID %in% c(12, 24, 37, 10, 32), "most", "least")),
          zcol = "type",
          legend = F,
    map.type = "Stamen.Toner",
    col.region = c("#d0587e", "#88CCEE")) + 
    mapview(combined_centroid, legend = FALSE,
            cex = 3) 
...
一般认为，一个地区在不同种族上的人口分布越平均，工资差距越小，种族多样程度也也该越好。然而，结合 2017 年美国社区调查提供的收入和人口统计，却能发现两个与常识相违背的趋势：一个洲内部各个种族人口总数差距越大，多样性却越好；同样，州内部的工资（中位数）分布越极端，多样性也越好。  
